package kr.co.jmena.www.web.home.buyingMng.Vo;

public class MM011001VO {
	
	private String BUYID_PK;
	private String BUYID;
	private String BUYDATE;
	private String BUYGUBUN;
	private String MANAGENO;
	private String CITYCODE;
	private String BOROUNGCODE;
	private String USETYPE;
	private String ADDRESS;
	private String OWNERNAME;
	private String OWNERJUMINID;
	private String BUYM2;
	private String BUYPY;
	private String BUYAMT;
	private String BUYDANGA;
	private String REGYN;
	private String REGDATE;
	private String REMARK;
	private String BUYSEQ;
	private String PAYGUBUN;
	private String PAYDATE;
	private String PAYAMT;
	private String PAYYN;
	private String BRANCHCODE;
	private String BRANCHNAME;
	private String OPENYN;
	private String HOLDINGYN;
	private String HOLDINGM2;
	private String HOLDINGPY;
	private String HOLDINGDATE;
	private String LS_BUYDATE_FR;
	private String LS_BUYDATE_TO;
	private String LS_INSERTUSER;
	private String LS_ADDRESS;
	private String USERID;
	private String CONBM2;
	private String CONJM2;
	private String CONBPY;
	private String CONJPY;
	private String PMDANGA;
	private String CITYNAME;
	private String BOROUGHCODE;
	private String BOROUGHNAME;
	private String BUNJANM2;
	private String BUNJANPY;
	private String CONM2;
	private String CONPY;

	public String getBUYID() {
		return this.BUYID;
	}

	public String getBUYDATE() {
		return this.BUYDATE;
	}

	public String getBUYGUBUN() {
		return this.BUYGUBUN;
	}
	
	public String getMANAGENO() {
		return this.MANAGENO;
	}
	
	public String getCITYCODE() {
		return this.CITYCODE;
	}
	
	public String getBOROUNGCODE() {
		return this.BOROUNGCODE;
	}
	
	public String getUSETYPE() {
		return this.USETYPE;
	}
	
	public String getADDRESS() {
		return this.ADDRESS;
	}
	
	public String getOWNERNAME() {
		return this.OWNERNAME;
	}
	
	public String getOWNERJUMINID() {
		return this.OWNERJUMINID;
	}
	
	public String getBUYM2() {
		return this.BUYM2;
	}
	
	public String getBUYPY() {
		return this.BUYPY;
	}
	
	public String getBUYAMT() {
		return this.BUYAMT;
	}
	
	public String getBUYDANGA() {
		return this.BUYDANGA;
	}
	
	public String getREGYN() {
		return this.REGYN;
	}
	
	public String getREGDATE() {
		return this.REGDATE;
	}
	
	public String getREMARK() {
		return this.REMARK;
	}

	public String getBUYSEQ(){
		return this.BUYSEQ;
	}
	
	public String getPAYGUBUN(){
		return this.PAYGUBUN;
	}
	
	public String getPAYDATE(){
		return this.PAYDATE;
	}
	
	public String getPAYAMT(){
		return this.PAYAMT;
	}
	
	public String getPAYYN(){
		return this.PAYYN;
	}
	
	public String getBRANCHCODE(){
		return this.BRANCHCODE;
	}
	public String getBRANCHNAME(){
		return this.BRANCHNAME;
	}
	public String getOPENYN(){
		return this.OPENYN;
	}
	public String getHOLDINGYN(){
		return this.HOLDINGYN;
	}
	public String getHOLDINGM2(){
		return this.HOLDINGM2;
	}
	public String getHOLDINGPY(){
		return this.HOLDINGPY;
	}
	public String getHOLDINGDATE(){
		return this.HOLDINGDATE;
	}

	public String getLS_BUYDATE_FR() {
		return this.LS_BUYDATE_FR;
	}

	public String getLS_BUYDATE_TO() {
		return this.LS_BUYDATE_TO;
	}

	public String getLS_INSERTUSER() {
		return this.LS_INSERTUSER;
	}

	public String getLS_ADDRESS() {
		return this.LS_ADDRESS;
	}

	public String getCITYNAME() {
		return this.CITYNAME;
	}

	public String getBOROUGHCODE() {
		return this.BOROUGHCODE;
	}

	public String getBOROUGHNAME() {
		return this.BOROUGHNAME;
	}

	public String getBUNJANM2() {
		return this.BUNJANM2;
	}

	public String getBUNJANPY() {
		return this.BUNJANPY;
	}

	
	public void setBUYID(String BUYID) {
		this.BUYID = BUYID;
	}

	public void setBUYDATE(String BUYDATE) {
		this.BUYDATE = BUYDATE;
	}

	public void setBUYGUBUN(String BUYGUBUN) {
		this.BUYGUBUN = BUYGUBUN;
	}
	
	public void setMANAGENO(String MANAGENO) {
		this.MANAGENO = MANAGENO;
	}
	
	public void setCITYCODE(String CITYCODE) {
		this.CITYCODE = CITYCODE;
	}
	
	public void setBOROUNGCODE(String BOROUNGCODE) {
		this.BOROUNGCODE = BOROUNGCODE;
	}
	
	public void setUSETYPE(String USETYPE) {
		this.USETYPE = USETYPE;
	}
	
	public void setADDRESS(String ADDRESS) {
		this.ADDRESS = ADDRESS;
	}
	
	public void setOWNERNAME(String OWNERNAME) {
		this.OWNERNAME = OWNERNAME;
	}
	
	public void setOWNERJUMINID(String OWNERJUMINID) {
		this.OWNERJUMINID = OWNERJUMINID;
	}
	
	public void setBUYM2(String BUYM2) {
		this.BUYM2 = BUYM2;
	}
	
	public void setBUYPY(String BUYPY) {
		this.BUYPY = BUYPY;
	}
	
	public void setBUYAMT(String BUYAMT) {
		this.BUYAMT = BUYAMT;
	}
	
	public void setBUYDANGA(String BUYDANGA)  {
		this.BUYDANGA = BUYDANGA;
	}
	
	public void setREGYN(String REGYN) {
		this.REGYN = REGYN;
	}
	
	public void setREGDATE(String REGDATE) {
		this.REGDATE = REGDATE;
	}
	
	public void setREMARK(String REMARK) {
		this.REMARK = REMARK;
	}

	public void setBUYSEQ(String BUYSEQ){
		this.BUYSEQ = BUYSEQ;
	}
	
	public void setPAYGUBUN(String PAYGUBUN){
		this.PAYGUBUN = PAYGUBUN;
	}
	
	public void setPAYDATE(String PAYDATE){
		this.PAYDATE = PAYDATE;
	}
	
	public void setPAYAMT(String PAYAMT){
		this.PAYAMT = PAYAMT;
	}
	
	public void setPAYYN(String PAYYN){
		this.PAYYN = PAYYN;
	}

	public void setBRANCHCODE(String BRANCHCODE){
		this.BRANCHCODE = BRANCHCODE;
	}
	public void setBRANCHNAME(String BRANCHNAME){
		this.BRANCHNAME = BRANCHNAME;
	}
	public void setOPENYN(String OPENYN){
		this.OPENYN = OPENYN;
	}
	public void setHOLDINGYN(String HOLDINGYN){
		this.HOLDINGYN = HOLDINGYN;
	}
	public void setHOLDINGM2(String HOLDINGM2){
		this.HOLDINGM2 = HOLDINGM2;
	}
	public void setHOLDINGPY(String HOLDINGPY){
		this.HOLDINGPY = HOLDINGPY;
	}
	public void setHOLDINGDATE(String HOLDINGDATE){
		this.HOLDINGDATE = HOLDINGDATE;
	}

	public void setLS_BUYDATE_FR(String LS_BUYDATE_FR) {
		this.LS_BUYDATE_FR = LS_BUYDATE_FR;
	}

	public void setLS_BUYDATE_TO(String LS_BUYDATE_TO) {
		this.LS_BUYDATE_TO = LS_BUYDATE_TO;
	}

	public void setLS_INSERTUSER(String LS_INSERTUSER) {
		this.LS_INSERTUSER = LS_INSERTUSER;
	}

	public void setLS_ADDRESS(String LS_ADDRESS) {
		this.LS_ADDRESS = LS_ADDRESS;
	}
	
	public void setCITYNAME(String CITYNAME) {
		this.CITYNAME = CITYNAME;
	}

	public void setBOROUGHCODE(String BOROUGHCODE) {
		this.BOROUGHCODE = BOROUGHCODE;
	}

	public void setBOROUGHNAME(String BOROUGHNAME) {
		this.BOROUGHNAME = BOROUGHNAME;
	}

	public void setBUNJANM2(String BUNJANM2) {
		this.BUNJANM2 = BUNJANM2;
	}

	public void setBUNJANPY(String BUNJANPY) {
		this.BUNJANPY = BUNJANPY;
	}

	public String getUSERID() {
		return USERID;
	}

	public void setUSERID(String USERID) {
		this.USERID = USERID;
	}

	public String getCONM2() {
		return CONM2;
	}

	public String getCONPY() {
		return CONPY;
	}

	public void setCONM2(String CONM2) {
		this.CONM2 = CONM2;
	}

	public void setCONPY(String CONPY) {
		this.CONPY = CONPY;
	}

	public String getCONBM2() {
		return CONBM2;
	}

	public String getCONJM2() {
		return CONJM2;
	}

	public String getCONBPY() {
		return CONBPY;
	}

	public String getCONJPY() {
		return CONJPY;
	}

	public void setCONBM2(String CONBM2) {
		this.CONBM2 = CONBM2;
	}

	public void setCONJM2(String CONJM2) {
		this.CONJM2 = CONJM2;
	}

	public void setCONBPY(String CONBPY) {
		this.CONBPY = CONBPY;
	}

	public void setCONJPY(String CONJPY) {
		this.CONJPY = CONJPY;
	}

	public String getBUYID_PK() {
		return BUYID_PK;
	}

	public void setBUYID_PK(String BUYID_PK) {
		this.BUYID_PK = BUYID_PK;
	}

	public String getPMDANGA() {
		return PMDANGA;
	}

	public void setPMDANGA(String PMDANGA) {
		this.PMDANGA = PMDANGA;
	}
	
}
