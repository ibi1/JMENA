package kr.co.jmena.www.web.home.buyingMng.Vo;

public class MM012001VO {
	
	private String CITYCODE;
	private String CITYNAME;
	private String BOROUGHCODE;
	private String BOROUGHNAME;
	private String ADDRESS;
	private String OWNERNAME;
	private String BUYM2;
	private String BUYPY;
	private String BUNBUYPY;
	private String JANBUYPY;
	private String BUYAMT;
	private String PAYAMT1;
	private String PAYDATE1;
	private String PAYAMT2;
	private String PAYDATE2;
	private String PAYAMT3;
	private String PAYDATE3;
	private String PAYAMT4;
	private String PAYDATE4;
	private String JANPAYAMT;
	private String OPENYN;
	private String REMARK;
	

	private String S_CITYCODE;
	private String S_BOROUGHCODE;
	private String S_ADDRESS;
	
	public String getCITYCODE() {
		return this.CITYCODE;
	}
	public String getCITYNAME() {
		return this.CITYNAME;
	}
	public String getBOROUGHCODE() {
		return this.BOROUGHCODE;
	}
	public String getBOROUGHNAME() {
		return this.BOROUGHNAME;
	}
	public String getADDRESS() {
		return this.ADDRESS;
	}
	public String getOWNERNAME() {
		return this.OWNERNAME;
	}
	public String getBUYM2() {
		return this.BUYM2;
	}
	public String getBUYPY() {
		return this.BUYPY;
	}
	public String getBUNBUYPY() {
		return this.BUNBUYPY;
	}
	public String getJANBUYPY() {
		return this.JANBUYPY;
	}
	public String getBUYAMT() {
		return this.BUYAMT;
	}
	public String getPAYAMT1() {
		return this.PAYAMT1;
	}
	public String getPAYDATE1() {
		return this.PAYDATE1;
	}
	public String getPAYAMT2() {
		return this.PAYAMT2;
	}
	public String getPAYDATE2() {
		return this.PAYDATE2;
	}
	public String getPAYAMT3() {
		return this.PAYAMT3;
	}
	public String getPAYDATE3() {
		return this.PAYDATE3;
	}
	public String getPAYAMT4() {
		return this.PAYAMT4;
	}
	public String getPAYDATE4() {
		return this.PAYDATE4;
	}
	public String getJANPAYAMT() {
		return this.JANPAYAMT;
	}
	public String getOPENYN() {
		return this.OPENYN;
	}
	public String getREMARK() {
		return this.REMARK;
	}
	
	public String getS_CITYCODE() {
		return this.S_CITYCODE;
	}
	public String getS_BOROUGHCODE() {
		return this.S_BOROUGHCODE;
	}
	public String getS_ADDRESS() {
		return this.S_ADDRESS;
	}
	
	public void setCITYCODE(String CITYCODE) {
		this.CITYCODE = CITYCODE;
	}
	public void setCITYNAME(String CITYNAME) {
		this.CITYNAME = CITYNAME;
	}
	public void setBOROUGHCODE(String BOROUGHCODE) {
		this.BOROUGHCODE = BOROUGHCODE;
	}
	public void setBOROUGHNAME(String BOROUGHNAME) {
		this.BOROUGHNAME = BOROUGHNAME;
	}
	public void setADDRESS(String ADDRESS) {
		this.ADDRESS = ADDRESS;
	}
	public void setOWNERNAME(String OWNERNAME) {
		this.OWNERNAME = OWNERNAME;
	}
	public void setBUYM2(String BUYM2) {
		this.BUYM2 = BUYM2;
	}
	public void setBUYPY(String BUYPY) {
		this.BUYPY = BUYPY;
	}
	public void setBUNBUYPY(String BUNBUYPY) {
		this.BUNBUYPY = BUNBUYPY;
	}
	public void setJANBUYPY(String JANBUYPY) {
		this.JANBUYPY = JANBUYPY;
	}
	public void setBUYAMT(String BUYAMT) {
		this.BUYAMT = BUYAMT;
	}
	public void setPAYAMT1(String PAYAMT1) {
		this.PAYAMT1 = PAYAMT1;
	}
	public void setPAYDATE1(String PAYDATE1) {
		this.PAYDATE1 = PAYDATE1;
	}
	public void setPAYAMT2(String PAYAMT2) {
		this.PAYAMT2 = PAYAMT2;
	}
	public void setPAYDATE2(String PAYDATE2) {
		this.PAYDATE2 = PAYDATE2;
	}
	public void setPAYAMT3(String PAYAMT3) {
		this.PAYAMT3 = PAYAMT3;
	}
	public void setPAYDATE3(String PAYDATE3) {
		this.PAYDATE3 = PAYDATE3;
	}
	public void setPAYAMT4(String PAYAMT4) {
		this.PAYAMT4 = PAYAMT4;
	}
	public void setPAYDATE4(String PAYDATE4) {
		this.PAYDATE4 = PAYDATE4;
	}
	public void setJANPAYAMT(String JANPAYAMT) {
		this.JANPAYAMT = JANPAYAMT;
	}
	public void setOPENYN(String OPENYN) {
		this.OPENYN = OPENYN;
	}
	public void setREMARK(String REMARK) {
		this.REMARK = REMARK;
	}

	public void setS_CITYCODE(String S_CITYCODE) {
		this.S_CITYCODE = S_CITYCODE;
	}
	public void setS_BOROUGHCODE(String S_BOROUGHCODE) {
		this.S_BOROUGHCODE = S_BOROUGHCODE;
	}
	public void setS_ADDRESS(String S_ADDRESS) {
		this.S_ADDRESS = S_ADDRESS;
	}

	
}
